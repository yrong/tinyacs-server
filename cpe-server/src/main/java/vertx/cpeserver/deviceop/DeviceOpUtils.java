...
import vertx.cpeserver.session.*;
...
public class DeviceOpUtils {
    ...
    public static void saveInProgressDeviceOp(
        ...
        final String payload = new JsonObject()
                .put(ConnectionRequestConstants.STATE, ConnectionRequestFsm.STATE_LOCKED)
                .put(CpeDeviceOp.FIELD_NAME_DEVICE_OP, deviceOp)
                .encode();
    ...
    public static void callback(
         ...
         */
        JsonObject payload = deviceOp.put(CpeDeviceOp.FIELD_NAME_STATE, state);
        if (result != null) {
            payload.put(CpeDeviceOp.FIELD_NAME_RESULT, result);
        }
        ...
        if (callbackUrl.startsWith(VertxConstants.URL_PROTOCOL_VERTX_EVENT_BUS)) {
            ...
            session.vertx.eventBus().send(address, payload, new DeliveryOptions().setSendTimeout(DEFAULT_CALLBACK_TIMEOUT), replyHandler);
